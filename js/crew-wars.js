...
async function loadActiveWars() {
  ...
  if (error) { list.innerHTML = errorStateHtml("Couldn't load wars right now.", 'loadActiveWars()'); refreshIcons(); logError(error); return; }
...
async function loadWarHistory() {
  ...
  if (error) { list.innerHTML = errorStateHtml("Couldn't load war history right now.", 'loadWarHistory()'); refreshIcons(); logError(error); return; }
...
async function loadWarLeaderboard() {
  ...
  if (error) { el.innerHTML = errorStateHtml("Couldn't load the leaderboard right now.", 'loadWarLeaderboard()'); refreshIcons(); logError(error); return; }
