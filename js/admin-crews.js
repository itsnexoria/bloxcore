...
async function loadCrewsTable() {
  ...
  if (error) { tbody.innerHTML = `<tr><td colspan="7" class="muted">Couldn't load crews.</td></tr>`; console.error(error); return; }
  if (!data.length) {
    tbody.innerHTML = '';
...
async function loadWarsTable() {
  ...
  if (error) { tbody.innerHTML = `<tr><td colspan="6" class="muted">Couldn't load wars.</td></tr>`; console.error(error); return; }
