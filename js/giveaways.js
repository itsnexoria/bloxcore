...
onReady(async () => {
  ...
  if (currentUser) {
    ...
    const fruitOrderMap = getFruitOrderMap();
    prizeItems = (items || []).slice().sort((a, b) => {
      if (a.category === 'fruit' && b.category === 'fruit') {
        const ai = fruitOrderMap.get(a.name.toLowerCase());
        const bi = fruitOrderMap.get(b.name.toLowerCase());
